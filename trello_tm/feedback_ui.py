...
class FeedbackUI(QMainWindow):
    ...
    def _create_ui(self):
        ...
        padding = (
            self.feedback_text.contentsMargins().top() + self.feedback_text.contentsMargins().bottom() + 5
        )  # 5 is extra vertical padding
    ...
    def _run_command(self):
        ...
        try:
            ...
            self._append_log(f"Error running command: {e!s}\n")
            self.run_button.setText("&Run")
...
def get_project_settings_group(project_dir: str) -> str:
    ...
    full_hash = hashlib.md5(project_dir.encode("utf-8")).hexdigest()[:8]  # noqa: S324
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt", default="I implemented the changes you requested.", help="The prompt to show to the user"
    )
